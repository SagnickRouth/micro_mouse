...
static void data(const uint8_t *p, uint16_t n)
{
    ...
    while (n) {
        ...
        memcpy(&b[1], p, k);
        HAL_I2C_Master_Transmit(&hi2c1, OLED_ADDR, b, k + 1, 100);
        p += k;
        n -= k;
    }
}
...
static void refresh(void)
{
    for (uint8_t page = 0; page < 8; page++) {
        ...
        cmd(0x00);
        cmd(0x10);
        data(&fb[page * OLED_WIDTH], OLED_WIDTH);
    }
}

void oled_init(void)
{
    /* I2C1/PB8/PB9 are initialized by MX_I2C1_Init() and the HAL MSP. */
    HAL_Delay(20);
